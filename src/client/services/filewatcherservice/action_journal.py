...
class ActionJournal:
    ...
    def add_change(self, op_code, old_path="", new_path="", folder_path=""):
        ...
        cursor.execute("INSERT INTO action_journal (op_code, old_path, new_path, folder_path, processed, in_process) VALUES (?, ?, ?, ?, ?, ?)", (op_code, old_path, new_path, folder_path, 0, 0))
        try:
            conn.commit()
            conn.close()
    ...
    def get_unprocessed_actions(self):
        ...
        cur.execute(
            ...
            FROM action_journal
            WHERE processed = 0 AND in_process = 0
            ORDER BY id
            LIMIT 50;
            """
        )
    ...
    def mark_in_process(self, ids):
        ...
        cur.execute(f"UPDATE action_journal SET in_process = 1 WHERE id IN ({placeholders})", ids)
