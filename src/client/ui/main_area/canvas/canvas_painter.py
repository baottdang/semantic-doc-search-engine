...
class CanvasPainter(QtWidgets.QWidget):
    ...
    def update_scale_image(self):
        if self.image:
            self.image = self.anchor_image.scaled(self.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def update_image(self, qimage):
        ...
        if qimage is not None:
            self.image = qimage.scaled(self.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation) 
