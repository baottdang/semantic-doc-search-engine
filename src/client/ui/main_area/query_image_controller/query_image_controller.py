...
class QueryImageController():
    ...
    def load_controller(self, path):
        ...
        if path != "":
            self.submit_file_to_display(path, 0)
    ...
    def load_image(self, image):
        ...
        if image is not None:
            self.submit_image_to_display(image)
    ...
    def submit_image_to_display(self, image):
        """
        ...
        """
        query_image_controller_signal_instance = get_query_image_controller_signal_instance()
        query_image_controller_signal_instance.done_get_query_pixmap_signal.emit(image, "QLEN-SCREENSHOT", str(1))
    ...
    def done_get_qimage_callback(self, future, path, page):
        ...
        """
        try:
            image, task_id = future.result()

            # Drop stale tasks
            if task_id != self._task_id:
                return
            
            if image is None and os.path.isfile(path):
                print("No image") # Placeholder for could not retrieve file, replace with Error Icon later
            else:
                query_image_controller_signal_instance = get_query_image_controller_signal_instance()
                query_image_controller_signal_instance.done_get_query_pixmap_signal.emit(image, path, str(page+1))
