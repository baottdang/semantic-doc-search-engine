...
- To achieve that, the project utilizes advance Machine Learning and CNN feature extraction models to learn and index the contents of the database, allowing searches to be based on **contextual similarity**, or **features** of the query file, returning either exact matches if found, or files of similar contents.
...
- AI-leveraged search engine: Allowing for senmatic based queries, where the user can initiate searches using real contents such as images or PDF files, and the returned results will be either exact matches of the query if found, or similar in contents.

- Fully crosstype: In this engine, PDF files and images are treated as one and the same from the user experience, where the content of an image can be use to search for the content a PDF file and vice versa, opening up many ways a user can tailor the tool to their need.

- Fast indexing: The index pipeline has been heavily optimized for maximum speed and performance while maintaining the lightweight status through multithreading and advance batching techniques with GPU-accelerated algorithms to provide a quick and smooth process.
